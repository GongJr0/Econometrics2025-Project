...
\begin{itemize}
    ...
    In our paper, we will rely on the Breusch--Godfrey test, VIF, and ACF/PACF analysis to check for evidence against weak exogeneity.
    Given the application of RESET is inappropriate for level based time-series data, as a final check, we will apploy the \textbf{Harvey\rule[0.5ex]{0.5em}{0.7pt}Collier Test} for to assess model specification.
...
\vspace{\baselineskip}
Finally, the Harvey--Collier test is applied to check for model specification. The test is performed by fitting $n-k$ simple regressions of $y$ using the design matrix $X$.
Each fitted model is used to calculate a one-step ahead prediction error, giving us a vector of errors $w$.
The test confirms $H_0: \mathbb{E}[w_i] = 0 \ \forall w$. This is achieved by computing:
...
\begin{equation}
    ...
    \begin{aligned}
        ...
        t_{HC} & = \bigg\lvert \frac{\bar{w}}{s_w / \sqrt{n-k}} \bigg\rvert \sim t_{n-k-1}
    \end{aligned}
    \label{eq:hc_stat}
\end{equation}