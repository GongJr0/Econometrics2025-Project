...
While ACF and PACF can directly be applied to the dependent variable, CCF results can be heavily distorted by non-stationarity and the presence of AR structures in the variables being compared.
...
The procedure transforms the exogenous variables into residuals of their $AR$ representation, then uses the $\beta$ coefficient to re-scale the dependent variable. 
...
\vspace{\baselineskip}
This transformation has no effect on the error profile of the fitted model as the distribution shapes and inter-variable interactions are perfectly preserved.\footnote{The statement holds for any non-penalized estimator of the form $\hat\beta = \arg\min_\beta L(y, X\beta)$, since rescaling $X$ can be absorbed into a reparameterization of $\beta$.}
