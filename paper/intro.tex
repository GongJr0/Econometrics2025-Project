...
Using the \textbf{Fischer Equation} we can relate inflation to nominal interest rates, and express output as a function of price levels.
\begin{equation}
    \begin{aligned}
        r_t &= i_t - \frac{\mathbb{E}_t[P_{t+1}] - P_t}{P_t} \\
        \Rightarrow y_t &= y^\star - \alpha(r_t - r_t^\star)
    \end{aligned}
    \label{eq:fischer_output}
\end{equation}
Computing the partial derivative and elasticity of output with respect to price levels yield:
\begin{equation}
    \begin{aligned}
        \frac{\delta y_t}{\delta P_t} &= -\frac{\alpha\mathbb{E}_t[P_{t+1}]}{P_t^2} \\
        \epsilon_{y_t, P_t} &= \frac{\alpha\mathbb{E}_t[P_{t+1}]}{P_t(P_tY^\star -\alpha(P_t(i_t-r^\star) + P_t - \mathbb{E}_t[P_{t+1}]))}
    \end{aligned}
    \label{eq:partial_output_price}
\end{equation}

\noindent Again, we see a negative absolute relationship and a non-zero elasticity between $P_t$ and $y_t$.

\vspace{\baselineskip}
The theoretical examinations show a clear relationship with defined interactions.
