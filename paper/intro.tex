...
there's an existing relationship by using multiple regression analysis. 
It shows a continuously existing correlation between GDP and unemployment rate in U.S. over fifty years. 
While the finding in (\cite{kitov_link_2021}) research shows the modified Okun's Law is an extraordinary predictive power and valid model.
They re-estimated the models of link between real GDP per capita and unemployment rate over eight developed countries.
Indifferent from previous research (\cite{mandel_relationship_2019}), they pointed out the real GDP per capita is nescesary to 
present the real economic growth while the price levels and unemployment rate are both real economic indicators.
It shows the accurate prediction of the dynamics of independent variable sine 1960s. 
